...
$.ajax({
    type: "GET",
    url: "http://123.207.110.102:8080/background/get-all-report-need",
    dataType: "json",
    async: false,
    success: function (data) {
        ...
        for (var i = 0; i < len; i++) {
            $("tbody").append(getHtmlContext(
                mydata[i].need.needId,
                mydata[i].reportNeed.reportStatus,
                mydata[i].need.needTitle,
                mydata[i].need.needDescription,
                mydata[i].need.needImg,
                mydata[i].reportNeed.needMessage,
                formatDateTime(mydata[i].reportNeed.createTime)
            ));
        }
